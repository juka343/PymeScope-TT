...
const fallbackInterpretationTitle = "Lectura de rotación de activos";
const fallbackInterpretationParagraphs = [
  "Los indicadores de rotación muestran qué tan eficientemente la empresa usa sus activos, cartera e inventarios para generar ingresos.",
  "En una empresa de servicios, estos indicadores ayudan a entender si los recursos disponibles están convirtiéndose en ventas y efectivo de forma eficiente. Una rotación baja puede indicar activos subutilizados, cobranza lenta o capital atrapado en cuentas por cobrar.",
];

const fallbackRecommendations = [
  {
    title: "Reducir el periodo de cobranza",
    description:
      "Revisar políticas de crédito, recordatorios de pago y seguimiento a clientes con saldos vencidos.",
    reason:
      "En servicios, cobrar tarde puede afectar directamente el flujo operativo aunque las ventas se vean altas.",
    priority: "media",
  },
  {
    title: "Optimizar el uso de activos fijos",
    description:
      "Evaluar si la infraestructura, equipo o herramientas actuales están generando ventas suficientes.",
    reason:
      "Activos subutilizados reducen eficiencia y pueden inmovilizar recursos que podrían destinarse a operación o crecimiento.",
    priority: "media",
  },
  {
    title: "Monitorear cuentas por cobrar",
    description:
      "Separar clientes por antigüedad de saldo y establecer acciones específicas para cada grupo.",
    reason:
      "Una cartera elevada puede inflar ventas en papel, pero dejar a la empresa sin liquidez real. La magia contable, tristemente, no paga nómina.",
    priority: "alta",
  },
];
...
const normalizeKpiValue = (value) => {
  if (value === "Sin Inventario" || value === "N/A" || value === null || value === undefined) {
    return "N/A";
  }
...
const fetchDashboardData = async () => {
  ...
  try {
    ...
    snapshot.forEach((docSnap) => {
      ...
      loadedPeriods.push({
        ...
        row: {
          ...
          ventas: formatCurrency(crudos.ventas_netas || 0),
          costo: formatCurrency(crudos.costo_ventas || 0),
          inventarios: formatCurrency(crudos.inventario || 0),
          cxc: formatCurrency(crudos.cuentas_por_cobrar || 0),
        },
      });
    });
    ...
    if (latest) {
      ...
      console.log("📊 KPIs DE ROTACIÓN DE ACTIVOS (MONOPERIODO):", latest.kpis);
    }
  } catch (error) {
...
<template>
  <div class="wrap">
    ...
    <div v-if="isLoading" class="loading">
      Cargando análisis de rotación de activos...
    </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="r in periodRows" :key="r.period">
                ...
                <td class="muted">{{ r.ventas }}</td>
                <td class="muted">{{ r.costo }}</td>
                <td class="muted">{{ r.inventarios }}</td>
                <td>
                  <span class="pill">{{ r.cxc }}</span>
                </td>
              </tr>
            </tbody>
          </table>
