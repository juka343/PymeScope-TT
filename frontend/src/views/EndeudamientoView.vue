...
const fallbackInterpretationTitle = "Lectura de endeudamiento";
const fallbackInterpretationParagraphs = [
  "Los indicadores de endeudamiento muestran qué tanto depende la empresa de recursos de terceros para financiar su operación.",
  "En una empresa de servicios, un nivel de deuda controlado ayuda a mantener flexibilidad financiera. Si la deuda crece demasiado, puede presionar el flujo de efectivo, limitar nuevas inversiones y reducir la capacidad para absorber meses flojos.",
];

const fallbackRecommendations = [
  {
    title: "Mantener deuda en niveles controlados",
    description:
      "Evitar contratar financiamiento adicional si no está ligado a proyectos con retorno claro.",
    reason:
      "En servicios, la capacidad de pago depende mucho del flujo mensual y de la estabilidad de los ingresos.",
    priority: "media",
  },
  {
    title: "Revisar costo financiero",
    description:
      "Comparar tasas, comisiones y condiciones de deuda vigente para detectar oportunidades de refinanciamiento.",
    reason:
      "Una tasa alta puede consumir utilidad operativa aunque el negocio venda bien.",
    priority: "media",
  },
  {
    title: "Fortalecer capital propio",
    description:
      "Reinvertir parte de las utilidades para reducir dependencia de terceros.",
    reason:
      "Una base patrimonial más sólida mejora estabilidad y capacidad de negociación ante bancos o proveedores.",
    priority: "media",
  },
];
...
const fetchDashboardData = async () => {
  ...
  try {
    ...
    snapshot.forEach((docSnap) => {
      ...
      const estructuraCrudos = estructura.datos_crudos || {};
      ...
      loadedPeriods.push({
        ...
        row: {
          ...
          capitalContable: formatCurrency(
            estructuraCrudos.capital_contable ||
              crudos.capital_contable ||
              crudos.capital_social ||
              0
          ),
        },
      });
    ...
    if (latest) {
      ...
      console.log("📊 KPIs DE ENDEUDAMIENTO (MONOPERIODO):", latest.kpis);
    }
  } catch (error) {
...
<template>
  <div class="wrap">
    ...
    <div v-if="isLoading" class="loading">
      Cargando análisis de endeudamiento...
    </div>
